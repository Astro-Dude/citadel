...
def trust_maintenance_score(state: TrustState) -> float:
    """
    Fraction of the episode where BOTH trust scores stayed ≥ 0.5.
    If no history yet, falls back to the current instantaneous state.
    """
    if not state.history_c2o or not state.history_o2c:
        return 1.0 if (state.trust_c2o >= 0.5 and state.trust_o2c >= 0.5) else 0.0
    total = len(state.history_c2o)
    good = sum(
        1 for c, o in zip(state.history_c2o, state.history_o2c)
        if c >= 0.5 and o >= 0.5
    )
