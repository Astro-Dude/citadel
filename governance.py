...
def _count_completed_destructive_chains(gstate: GovernanceState) -> int:
    """
    Count how many destructive actions (isolate / block_external) on data
    systems were preceded by all required prereqs — i.e. the CAB approval
    and data-owner notification both happened BEFORE the SOX log entry
    recording the destructive action.

    This is the signal that separates "agent actually followed the procedure"
    from "agent spammed a bunch of governance calls".
    """
    completed = 0
    for entry in gstate.sox_log:
        action = entry.get("action")
        target = entry.get("target")
        if action not in DESTRUCTIVE_BASTION_ACTIONS or not target:
            continue
        if target not in DATA_HOLDING_SYSTEMS:
            # isolate on a non-data system — no prereqs needed; count as a chain
            # but with smaller weight (we treat it as half-chain)
            completed += 0  # not a strict chain; ignore here
            continue
        # Must have CAB approval keyed by (action, target) AND owner-notify
        cab_key = gstate.cab_key(action, target)
        if cab_key in gstate.cab_approvals and target in gstate.data_owners_notified:
            # Both prereqs must have been recorded BEFORE the SOX log hour
            sox_hour = entry.get("hour", 0)
            if (
                gstate.cab_approvals[cab_key] <= sox_hour
                and gstate.data_owners_notified[target] <= sox_hour
            ):
                completed += 1
    return completed
...
def compliance_score(gstate: GovernanceState, had_p1: bool = False) -> float:
    """
    Governance compliance, in [0, 1].

    Favors *completed chains* — destructive actions that were preceded by
    CAB approval, data-owner notification, and recorded in the SOX audit log.
    A spray of unrelated governance events earns little; zero violations and
    no structured engagement earn a modest baseline.

    Components:
      * chain_score      — completed prereq chains (dominant signal)
      * engagement_score — at-least-some governance activity happened
      * violation_hit    — subtracts hard for each recorded violation

    Mild P1 bonus when the episode involved a P1 incident (reflects the
    higher bar — you had to respond, ticket, page, and post, correctly).
    """
    viols = len(gstate.violations)
    hits = len(gstate.compliance_hits)
    chains = _count_completed_destructive_chains(gstate)

    # If no destructive actions were taken this episode, reward proportionally
    # to engagement + having no violations — we don't want to require chains
    # when none were needed.
    any_destructive_logged = any(
        entry.get("action") in DESTRUCTIVE_BASTION_ACTIONS
        for entry in gstate.sox_log
    )

    if any_destructive_logged:
        # 0 chains → 0.0, 1 chain → 0.70, 2 chains → 0.90, 3+ → 1.0
        if chains == 0:
            chain_score = 0.0
        elif chains == 1:
            chain_score = 0.70
        ...
        # No destructive actions — chains aren't applicable. Use engagement.
        chain_score = 0.5 if hits >= 2 else 0.3

    # Engagement: a small bonus for having DONE the work (tickets, notifs, etc.)
    engagement = min(1.0, hits / 6.0)     # 6 hits ≈ full engagement
    engagement_bonus = 0.15 * engagement

    # Violations: each one costs 0.15, capped
    violation_penalty = min(0.60, 0.15 * viols)

    score = chain_score + engagement_bonus - violation_penalty

    # Minor P1 difficulty bonus
    if had_p1 and viols == 0:
        score += 0.05
