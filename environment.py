...
class CitadelEnvironment(Environment[IncidentAction, IncidentObservation, IncidentState]):
    ...
    def __init__(self, oversight_policy: Optional[OversightPolicy] = None) -> None:
        super().__init__()
    ...
    def reset(
        ...
        **kwargs: Any,
        ...
        adversary_gen = kwargs.get("adversary_gen", None)
        ...
        **kwargs: Any,
        ...
        if applied:
            if is_bastion_action(proposal.action):
                ...
                governance_prereq_violations = check_prerequisites(
        ...
        # 7. Adversary turn (always advances)
        new_alerts = adversary_turn(self._state, self._rng, generation=self._state.adversary_gen)
        ...
        # 13. Write post-mortem lesson
        lesson_utility_delta = 0.0
        if oversight_action.lesson_text.strip():
            tgt_sys = (
                SYSTEM_NAMES[proposal.target_system]
                if 0 <= proposal.target_system < len(SYSTEM_NAMES) else ""
            )
